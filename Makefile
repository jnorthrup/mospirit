...
BOOST_PREFIX?=/c/work/boost_
BOOST_VER?=1_55_0
BOOST_INC?=$(BOOST_PREFIX)$(BOOST_VER)/
INCLUDES+= -I$(BOOST_INC)

BOOST_LIB?=$(BOOST_PREFIX)/libs
...
%.cpp  %.hpp : %.ypp
	$(YACC) -o $(@:.hpp=.cpp) $(@:.hxx=.cxx) $<
	$(SED) -i~ 's,# define PARSER_HEADER_H,#pragma once,' $(@:.cpp=.hpp) $(@:.cxx=.hxx)
...
all: dep  Mlite.exe #quadbag.exe hatsort.exe
